// Example echoauth shows the full tenant-auth wiring for Echo: an operator
// signs an account token carrying an HTTP extension, the server installs the
// echoauth middleware on the app, and the client signs every request via the
// framework-agnostic httpauth transport. Runs self-contained against a local
// listener.
package main
...
func main() {
	...
	app := echo.New()
	app.HideBanner = true
	app.Use(echoauth.NewMiddleware(valiss.NewVerifier(operatorPub, valiss.NewStaticAllowlist(claims.ID))))
	app.GET("/v1/whoami", func(c echo.Context) error {
		...
		if err != nil || !ok {
			return echo.NewHTTPError(http.StatusForbidden, "no query filters delegated")
		}
		log.Printf("tenant %q calls %s with region filters %v", id.Account.Name, c.Request().URL.Path, filters.Regions)
		return c.String(http.StatusOK, fmt.Sprintf("hello, tenant %q; your queries are filtered to regions %v\n", id.Account.Name, filters.Regions))
	})
