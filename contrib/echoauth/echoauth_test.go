...
// newServer builds an Echo app with the middleware and a handler echoing
...
func newServer(t *testing.T, verifier *valiss.Verifier, opts ...httpauth.Option) *httptest.Server {
	...
	echoIdentity := func(c echo.Context) error {
		id, ok := IdentityFrom(c)
		if !ok {
			return echo.NewHTTPError(http.StatusInternalServerError, "no identity in context")
		}
...
func TestMiddleware(t *testing.T) {
	...
	t.Run("missing credential rejected with 401", func(t *testing.T) {
...
func TestExtEnforcement(t *testing.T) {
	...
	t.Run("path outside the extension rejected with 403", func(t *testing.T) {
	...
	t.Run("method outside the extension rejected with 403", func(t *testing.T) {
...
func TestUserChain(t *testing.T) {
	...
	t.Run("path beyond the delegation rejected", func(t *testing.T) {
...
// TestRejectionStopsChain proves a rejection prevents the handler from
// running and surfaces through Echo's error handler.
func TestRejectionStopsChain(t *testing.T) {
	_, opPub := issuerKeys(t)

	var reached bool
	e := echo.New()
	e.Use(NewMiddleware(valiss.NewVerifier(opPub, valiss.AllowAll{})))
	e.GET("/", func(c echo.Context) error { reached = true; return nil })
	srv := httptest.NewServer(e)
	defer srv.Close()
	...
	resp.Body.Close()
	assert.Equal(t, http.StatusUnauthorized, resp.StatusCode)
	assert.False(t, reached, "handler must not run after rejection")
}
