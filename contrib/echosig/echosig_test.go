...
// newServer builds an Echo app with the middleware and a handler capturing
...
func TestMiddleware(t *testing.T) {
	...
	t.Run("tampered body rejected with 401", func(t *testing.T) {
		var tok string
...
// Echo error path: a negotiating transport gets the valiss-chain: required
...
func TestChainNegotiation(t *testing.T) {
	...
	t.Run("unknown bare token rejected with the signal", func(t *testing.T) {
		...
		// through the Echo error path.
		fresh := echo.New()
		fresh.Use(NewMiddleware(opPub, httpsig.WithChainCache(valiss.NewMemoryChainCache())))
		fresh.POST("/*", func(c echo.Context) error { return c.NoContent(http.StatusOK) })
		...
		body, _ := io.ReadAll(resp.Body)
		assert.Equal(t, http.StatusUnauthorized, resp.StatusCode)
		assert.Equal(t, valiss.ChainRequired, resp.Header.Get(valiss.HeaderChain), "error path carries the negotiation signal")
