...
// tokens the operator explicitly deposited server-side pass, so a token can
...
func LoadAllowlistFile(path string) (*StaticAllowlist, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("tokenator: open allowlist: %w", err)
	}
	defer f.Close()
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("tokenator: read allowlist: %w", err)
	}
