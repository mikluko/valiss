...
- `pkg/grpcauth`, `pkg/httpauth` — thin transport adapters over `token.Verifier`: header/metadata extraction, error mapping (gRPC status codes / HTTP 401/403), and client-side attachment (per-RPC credentials / `http.RoundTripper`), both constructed from a `creds.Creds`. Headers shared by both transports: `valiss-tenant-token`, `valiss-user-token`, `valiss-tenant-timestamp`, `valiss-tenant-signature` (`token.Header*` constants).
