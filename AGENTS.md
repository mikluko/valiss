...
Tokens are valiss's own typed claims (claims.go): RFC 7519 standard fields plus a `valiss` section (`type`, `epoch`, `bearer`, and named `ext` extensions). `sub` is the subject's public key (no keyless subjects) and `name` carries the optional human label (`WithName` at mint on operator, account, and user tokens — never message tokens; an unnamed entity is represented by its public key). Names are issuer-asserted with no uniqueness check at issuance: a collection holding several entities (anchor keyring, tenant directory) owns uniqueness in its scope. Validity is absolute and optional: no `exp` = never expires; `nbf` supported. Go-side, the base `Claims` struct is RFC-only (jti/iss/sub/iat/exp/nbf); `OperatorClaims{Claims, Name, Epoch, Ext}`, `AccountClaims{Claims, Name, Epoch, Ext}` and `UserClaims{Claims, Name, Epoch, Bearer, Ext}` embed it via the golang-jwt-style registered-claims pattern.
...
- root — token issue/verify (`IssueAccount`/`IssueUser`/`IssueMessage`, `VerifyAccount`/`VerifyUser`/`VerifyMessage`/`Decode`; `Issue` is the deprecated alias of `IssueAccount`), `SignRequest`/`VerifySignature`, `Allowlist`, `Verifier`, extension plumbing, `IdentityFromContext`. `VerifyAccount`/`VerifyUser` deliberately do NOT check expiry or allowlist; `Verifier` layers those so callers get precise errors. `Decode` returns RFC-only `Claims` without establishing trust (tooling).
...
- `contrib/httpsig`, `contrib/grpcsig` — message-token (proof-of-origin) adapters, carrying the token in the `valiss-message-token` header: httpsig `NewTransport`/`NewMiddleware` (audience = host + path, checksum over the body), grpcsig `UnaryClientInterceptor`/`UnaryServerInterceptor` (audience = full method, checksum over the deterministic protobuf encoding). Emitters need bundle creds with the user seed, the epoch is derived from the chain tokens (which must agree), and handlers read claims with `valiss.MessageFromContext`. Server sides take package options (`WithVerifyOptions` passes `valiss.VerifyMessageOption`s through; `WithChainCache` enables cached chain negotiation). Chain negotiation (`WithChainNegotiation` on the client side) sends bare tokens and retransmits once with the chain in the detached `valiss-chain-*` headers when the receiver answers the `valiss-chain: required` signal (response header on HTTP, trailer on gRPC); `valiss.ErrNoChain` is the sentinel that drives it, and only fully verified chains enter a `valiss.ChainCache`.
